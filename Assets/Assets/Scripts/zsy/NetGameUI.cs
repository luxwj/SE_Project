...
public class NetGameUI : GameUI {

    public NGameController m_NgameController;
    ...
    private INetworkTransform iNetworkTransform;
    ...
    public override void OnStartButtonClicked()
    {
        ...
        GameObject clientBat = GameObject.FindWithTag("NetPlayer").gameObject;
        iNetworkTransform = clientBat.GetComponent<NetworkTransformFacade>() as INetworkTransform;
        m_NgameController.StartGame();
        if (tutorials)
        {
    ...
    private void SetClientInput()
    {
        ...
        Debug.Log(h);
        if (iNetworkTransform != null)
        {
            Debug.Log("1");
            iNetworkTransform.SetInput(h, v, bat);
        }
    }
}
